...
class RepairPressHelpers {
	...
	static function pretty_comments_number() {
		...
		printf(
			...
			_n( '%s Comment', '%s Comments', get_comments_number(), 'repairpress-pt' ), number_format_i18n( get_comments_number() )
		);
	}
	...
	static function google_web_fonts_url() {
		...
		$fonts = apply_filters( 'repairpress_pre_google_web_fonts', $fonts );
		...
		$subset = _x( 'no-subset', 'Add new subset (greek, cyrillic, devanagari, vietnamese)', 'repairpress-pt' );
		if ( 'cyrillic' == $subset ) {
		...
		$subsets = apply_filters( 'repairpress_subsets_google_web_fonts', $subsets );
		...
		return apply_filters( 'repairpress_google_web_fonts_url', $fonts_url );
	}
				...
				<div class="comment-inner">
					...
					<div class="comment-metadata">
						<time datetime="<?php comment_time( 'c' ); ?>">
						 	<?php printf( _x( '%1$s at %2$s', '1: date, 2: time' , 'repairpress-pt' ), get_comment_date(), get_comment_time() ); ?>
						</time>
						<?php comment_reply_link( array_merge( $args, array(
							'depth' => $depth,
							'before' => ' / ',
						) ) ); ?>
						<?php edit_comment_link( esc_html__( 'Edit', 'repairpress-pt' ), ' / ' ); ?>
					</div>
					<div class="comment-text">
						<?php if ( '0' == $comment->comment_approved ) : ?>
						<p class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.' , 'repairpress-pt' ); ?></p>
						<?php endif; ?>
